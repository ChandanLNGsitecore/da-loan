...
export default function FileUpload() {
  ...
  const handleFile = (file: File | undefined) => {
    ...
    const validExtensions = ["png", "jpg", "jpeg", "heic", "pdf"];
    const isValid =
    ...
    if (isValid) {
      ...
      toast.error("Please upload a PNG, JPG, HEIC, or PDF file.", {
  ...
  const getFileIcon = () => {
    ...
    if (["png", "jpg", "jpeg", "heic"].includes(fileExt)) {
      return <Image className="h-5 w-5 text-foreground" aria-hidden={true} />;
    } else if (fileExt === "pdf") {
    ...
    <div className="flex items-center justify-center p-10 w-full max-w-lg">
      <form className="w-full" onSubmit={(e) => e.preventDefault()}>
        ...
        <p className="mt-2 text-xs leading-5 text-muted-foreground sm:flex sm:items-center sm:justify-between">
          <span>Accepted file types: PNG, JPG, HEIC or PDF files.</span>
          ...
          <Card className="relative mt-8 bg-muted p-4 gap-4">
            ...
            <div className="flex items-center space-x-2.5">
              ...
              <div>
                ...
                <p className="mt-0.5 text-xs text-muted-foreground">2.4 MB</p>
              </div>
            </div>
